...
func (t *Transfer) ReadPkg() (mes message.Message, err error) {
	...
	fmt.Println("读取客户端发送的数据, 我是服务端的utils......")
	_, err = t.Conn.Read(t.Buf[:4]) // 读取四个,这里会阻塞
	if err != nil {
		if err == io.EOF {
			fmt.Println("客户端退出触发EOF")
			return
		} else {
	...
	err = json.Unmarshal(t.Buf[:pkgLen], &mes) // 一定要记得加&,如果不加,mes是空的
	if err != nil {
